        ...
        return this.each(function (index) {
            ...
            function uploadSubmitHandler () {
                if (state.fileBatch.length !== 0) {
                    ...
                    data.append('mainPicId',document.getElementById('mainPicId').value);
                    data.append('cardId',document.getElementById('cardId').value);
                    for (var i = 0; i < state.fileBatch.length; i++) {
                        data.append('files[]', state.fileBatch[i].file, state.fileBatch[i].fileName);
                    }
                    //alert('图片上传中，请等待上传成功提示:'+document.getElementById('mainPicId').value);
                    $.ajax({
                        type: 'POST',
                        url: options.ajaxUrl,
                        data: data,
                        ...
                        success: function(data){
    				    	window.location.href='cardlist.html?static=t&cardId='+document.getElementById('cardId').value+'#slide-'+document.getElementById('slide').value;
    				    }
                    });
                }
            }
