...
public class MyplanServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("edit") != null) {
			Myplan m = dao.get(Long.parseLong(request.getParameter("planId")));
			request.setAttribute("myplan", m);
		}

		if (request.getParameter("delete") != null) {
			dao.delete(Long.parseLong(request.getParameter("planId")), kehuId);
		}

		Collection plancoll = dao.getAll(kehuId);
